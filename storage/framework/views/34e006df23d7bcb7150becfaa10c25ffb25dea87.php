                    ...
                    <li>
                        <div class="form-group">
                            <select name="" id="teachers_drop_down" class="w-100">
                                <?php $__currentLoopData = $get_all_teacher; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $teacher): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<script type="text/javascript">
    ...
    function get_slots(defaultSlot, append_id) {
        $.ajax({
            url: "<?php echo e(route('admin_side_get_slots')); ?>",
            type: 'GET',
            headers: {
    ...
    $(document).on('click', '.a_slots', function() {
        ...
        $.ajax({
            url: "<?php echo e(route('admin_all_students_details')); ?>",
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            ...
            success: function(data) {
                var students = '<form method="post" action="<?php echo e(route("admin_student_slot")); ?>"><div><select  required class="form-control" name="student_id"><option value="">Select Student</option>';
                $.each(data.students, function(k, v) {
                    students += '<option value="'+v.id+'">'+v.name+'</option>';
                })
                students += '</select></div>';
                students += '<input name="_token" type="hidden" value="<?php echo e(csrf_token()); ?>" /><input name="start" type="hidden" value="'+start+'" /><input name="time" type="hidden" value="'+time+'"  /><input type="hidden" name="teacher_id" value="'+teacher_id+'"  /><div class="card_detail w-100 mt-3">\n\
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\xampp\htdocs\booking-sheduler\resources\views/dashboard/booking_calender/admin_calender.blade.php ENDPATH**/ ?>