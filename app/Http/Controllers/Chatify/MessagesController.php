...
class MessagesController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $records = User::where('id', 0)->where('name', 'LIKE', "%{$input}%");
        ...
        if (auth()->user()->hasRole('Student')) {
            ...
            $records = User::where('id', $data->user_id)->where('name', 'LIKE', "%{$input}%");
        }

        if (auth()->user()->hasRole('Teacher')) {
            $data = Lesson::where('user_id', Auth()->user()->id)
                ->get('students')->toArray();
            $records = User::whereIn('id', $data)->where('name', 'LIKE', "%{$input}%");
            // dd($records);
        }
